...
* 扩容
* ![1761641842212](image/集合-源码分析/1761641842212.png)
...
* `add(E e)`和 `offer(E e)`：前者在插入失败时抛出异常，后则则会返回 `false`

![1761642221844](image/集合-源码分析/1761642221844.png)
...
* `remove()`和 `poll()`：获取并删除队首元素

![1761645073235](image/集合-源码分析/1761645073235.png)
...
* `remove(Object o)`：删除队列中跟 `o`相等的某一个元素（如果有多个相等，只删除第一个）

  * 删除的是最后一个元素。直接删除即可，不需要调整。
  * 删除的不是最后一个元素，从删除点开始以最后一个元素为参照调用一次 `siftDown()`即可。此处不再赘述。

![1761645205171](image/集合-源码分析/1761645205171.png)
