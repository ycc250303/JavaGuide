...
* 用于描述程序在运行时复杂的流程控制
* 描述多个类或对象之间怎样相互协作完成单个对象无法完成的任务
* 涉及算法和对象间职责的分配
* 分为类行为模式和对象行为模式

## 模板方法模式

* 定义了一种算法框架， 允许子类在不修改结构的情况下重写算法的特定步骤。
* 操作顺序固定

### 角色

![1765851392272](image/小浣熊+heima/1765851392272.png)

* 抽象类：给出算法的轮廓和骨架
  * 模板方法：定义算法的骨架，按某种顺序调用其包含的基本方法
  * 基本方法：设计案算法各个步骤的方法，模板方法的组成部分
    * 抽象方法
    * 具体方法
...
* 缺点
  ...
  * 收到算法框架限制
  * 每个实现都需要定义子类
  * 通过子类抑制默认步骤实现可能会导致违反 *里**氏**替**换**原**则*** **。**
...
![1765854799532](image/小浣熊+heima/1765854799532.png)

### 优缺点

* 优点
  * 策略类之间可以自由切换
  * 符合开闭原则，无需修改上下文即可引入新的策略
  * 可以在运行时切换对象内的算法
* 缺点
  * 客户端必须知道所有策略类
  * 如果算法很少发生改变，这种模式会让算法变复杂
...
* 定义一个中介角色。限制对象之间的直接交互，迫使它们通过一个中介者对象进行合作。

### 角色

![1765867848224](image/小浣熊+heima/1765867848224.png)

### 优缺点

* 优点
  * 单一职责，可以将多个组件的交流抽取到同一位置
...
* 提供一个对象顺序访问聚合对象中的一系列数据，而不暴露对象的内部表示

### 角色

![1765870496269](image/小浣熊+heima/1765870496269.png)
...
* 集合背后存在复杂数据结构，且希望对客户端隐藏复杂性
* 为聚合对象提供多种遍历方式
* 为遍历不同的聚合结构提供统一接口

## 命令模式

* 将请求封装为对象，使发出请求的责任和执行请求的责任分隔开。两者通过命令对象进行沟通
* 该转换让你能根据不同的请求将方法参数化、延迟请求执行或将其放入队列中，且能实现可撤销操作。

### 角色

![1765961898286](image/小浣熊+heima/1765961898286.png)
...
![1765964329313](image/小浣熊+heima/1765964329313.png)
